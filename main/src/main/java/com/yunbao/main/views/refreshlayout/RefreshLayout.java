...
package com.yunbao.main.views.refreshlayout;
...
public class RefreshLayout extends ViewGroup {
    ...
    private RefreshLayoutDirection mDirection;
    ...
    private AnimationListener mRefreshListener = new AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            if (mRefreshing) {
                ...
                mProgress.start();
                if (mNotify) {
                    if (mListener != null && mDirection == RefreshLayoutDirection.BOTTOM) {
                        mListener.onPullUpToRefresh();
                    } else if (mListener != null && mDirection == RefreshLayoutDirection.TOP) {
    ...
    public RefreshLayout(Context context, AttributeSet attrs) {
        ...
        final TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.RefreshLayout);
        RefreshLayoutDirection direction
                = RefreshLayoutDirection.getFromInt(a2.getInt(R.styleable.RefreshLayout_direction, 0));
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mBothDirection) {
                    ...
                    if ((mDirection == RefreshLayoutDirection.BOTTOM && canChildScrollDown())
                            || (mDirection == RefreshLayoutDirection.TOP && canChildScrollUp())) {
                        mInitialDownY = y;
                        return false;
                    }
                }
...
//        public void onRefresh(RefreshLayoutDirection direction);
    ...
    public RefreshLayoutDirection getDirection() {
        return mBothDirection ? RefreshLayoutDirection.BOTH : mDirection;
    }

    public void setDirection(RefreshLayoutDirection direction) {
        if (direction == RefreshLayoutDirection.BOTH) {
            mBothDirection = true;
        } else {
            mBothDirection = false;
    ...
    // only TOP or Bottom
    private void setRawDirection(RefreshLayoutDirection direction) {
    ...
    public void onLoad(int size) {
        ...
        if (size < pageSize) {
            setDirection(RefreshLayoutDirection.TOP);
        } else {
            setDirection(RefreshLayoutDirection.BOTH);
        }
    }
